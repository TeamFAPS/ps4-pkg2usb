...
static int config_handler(void* user, const char* name, const char* value)
{
	...
	if (MATCH("title_id")) {
		pconfig->title_id = chop(strdup(value));
	} else
    if (MATCH("copy")) {
        pconfig->copy = atoi(value);
    } else
    if (MATCH("notify")) {
...
int _main(struct thread *td)
{
	...
	notify("Welcome to ps4-pkg2usb v"VERSION);
	sceKernelSleep(5);
    ...
    sprintf(cfg_path, "%s/%s", usb_path, CONFIG_FILE);

    if (!file_exists(cfg_path)) {
        sprintf(msg, "Error: %s is missing!", CONFIG_FILE);
        notify(msg);
        return -1;
    }
	...
	sprintf(title_id, config.title_id);

	if (strlen(title_id) == 0) {
		sprintf(msg, "Error: title_id is missing in config!");
		notify(msg);
		return -1;
	}

	link_pkg(title_id, usb_path);
